...
--1) *** user input variables ***
IF OBJECT_ID('tempdb..#tmp_cm22_user_input', 'U') IS NOT NULL		DROP TABLE #tmp_cm22_user_input;
/* 
-------------------------
	START user input
...
--2) clear leftover temp tables

IF OBJECT_ID('tempdb..#tmp_cm22_vital_concepts', 'U') IS NOT NULL 	DROP TABLE #tmp_cm22_vital_concepts;
IF OBJECT_ID('tempdb..#tmp_cm22_smoking_concepts', 'U') IS NOT NULL DROP TABLE #tmp_cm22_smoking_concepts;
IF OBJECT_ID('tempdb..#tmp_cm22_c1yr', 'U') IS NOT NULL 			DROP TABLE #tmp_cm22_c1yr;
IF OBJECT_ID('tempdb..#tmp_cm22_c5yr', 'U') IS NOT NULL 			DROP TABLE #tmp_cm22_c5yr;
IF OBJECT_ID('tempdb..#tmp_cm22_concept_checks', 'U') IS NOT NULL 	DROP TABLE #tmp_cm22_concept_checks;
IF OBJECT_ID('tempdb..#concept_checks_1yr', 'U') IS NOT NULL 		DROP TABLE #concept_checks_1yr;
IF OBJECT_ID('tempdb..#concept_checks_5yr', 'U') IS NOT NULL 		DROP TABLE #concept_checks_5yr;
...
-- smoking concepts
SELECT conc.*
INTO #tmp_cm22_smoking_concepts
...
SELECT x.concept_id as src_concept_id
...
-- Concept check - 1yr
SELECT src_vocabulary_id, count(distinct cc.src_concept_code) as denominator, count(distinct x.src_concept_id) as numerator 
INTO #concept_checks_1yr 
FROM #tmp_cm22_concept_checks cc
LEFT OUTER JOIN
(
...
SELECT 
	...
	,(SELECT TOP 1 data_model from #tmp_cm22_user_input) as one_year -- 2 = OMOP
	,(SELECT TOP 1 data_model from #tmp_cm22_user_input) as five_year -- 2 = OMOP
UNION
SELECT
...
,(
	...
	(
		...
		SELECT DISTINCT person_id 
		FROM person 
		WHERE DATEDIFF(year, birth_datetime, '12-31-2022') > 120
