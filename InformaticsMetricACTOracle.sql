...
SELECT
  'total_encounters' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.ENCOUNTER_NUM) CNT
        FROM OBSERVATION_FACT OBS WHERE OBS.START_DATE BETWEEN '01-JAN-2021' AND '31-DEC-2021'
    ) AS ONE_YEAR,
    
    ( SELECT COUNT(DISTINCT OBS.ENCOUNTER_NUM) CNT
        FROM OBSERVATION_FACT OBS WHERE OBS.START_DATE BETWEEN '01-JAN-2017' AND '31-DEC-2021'
    ) AS FIVE_YEAR
FROM DUAL;
COMMIT;
...
SELECT
  'uniq_pt_with_gender' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        ...
        WHERE OBS.START_DATE BETWEEN '01-JAN-2021' AND '31-DEC-2021'
    ) AS ONE_YEAR,
    
    ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        FROM OBSERVATION_FACT OBS
        JOIN PATIENT_DIMENSION PAT ON PAT.PATIENT_NUM = OBS.PATIENT_NUM AND (PAT.SEX_CD IS NULL OR PAT.SEX_CD <> 'DEM|SEX:NI')
        WHERE OBS.START_DATE BETWEEN '01-JAN-2017' AND '31-DEC-2021'
    ) AS FIVE_YEAR
FROM DUAL;
COMMIT;
...
SELECT
  'uniq_pt_age_over_120' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        FROM OBSERVATION_FACT OBS
        JOIN PATIENT_DIMENSION PAT ON PAT.PATIENT_NUM = OBS.PATIENT_NUM 
        AND ((PAT.DEATH_DATE IS NULL AND TO_DATE('31-DEC-2021', 'DD-MON-YYYY') - PAT.BIRTH_DATE > 120*365)
...
SELECT
  'uniq_pt_snomed_proc' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        ...
        JOIN OBSERVATION_FACT OBS ON OBS.CONCEPT_CD = MAP.CONCEPT_CD AND OBS.START_DATE BETWEEN '01-JAN-2021' AND '31-DEC-2021'
    ) AS ONE_YEAR,
    
    ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        FROM MAPPED_DOMAIN MAP
        JOIN OBSERVATION_FACT OBS ON OBS.CONCEPT_CD = MAP.CONCEPT_CD AND OBS.START_DATE BETWEEN '01-JAN-2017' AND '31-DEC-2021'
    ) AS FIVE_YEAR
FROM DUAL;
COMMIT;
