...
SELECT
  'uniq_pt_with_gender' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        ...
        WHERE OBS.START_DATE BETWEEN '01-01-2020' AND '12-31-2020'
    ) AS ONE_YEAR,
    
    ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        FROM OBSERVATION_FACT OBS
        JOIN PATIENT_DIMENSION PAT ON PAT.PATIENT_NUM = OBS.PATIENT_NUM AND (PAT.SEX_CD IS NULL OR PAT.SEX_CD <> 'DEM|SEX:NI')
        WHERE OBS.START_DATE BETWEEN '01-01-2016' AND '12-31-2020'
    ) AS FIVE_YEAR
;
COMMIT;
  ...
  'total_pt_gt_12' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        ...
        JOIN PATIENT_DIMENSION PAT ON PAT.PATIENT_NUM = OBS.PATIENT_NUM 
            AND PAT.BIRTH_DATE IS NOT NULL AND DATEDIFF(YEAR, PAT.BIRTH_DATETIME, OBS.START_DATE) > 12 
        WHERE OBS.START_DATE BETWEEN '01-01-2020' AND '12-31-2020'
    ) AS ONE_YEAR,
...
SELECT
  'uniq_pt_age_over_120' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        FROM OBSERVATION_FACT OBS
        JOIN PATIENT_DIMENSION PAT ON PAT.PATIENT_NUM = OBS.PATIENT_NUM 
        AND ((PAT.DEATH_DATE IS NULL AND DATEDIFF(YEAR, PAT.BIRTH_DATETIME, '12-31-2020')  > 120)
            OR (DATEDIFF(YEAR, PAT.BIRTH_DATETIME, PAT.DEATH_DATE)  > 120))
        WHERE OBS.START_DATE BETWEEN '01-01-2020' AND '12-31-2020'
    ) AS ONE_YEAR,
...
SELECT
  'uniq_pt_snomed_proc' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        ...
        JOIN OBSERVATION_FACT OBS ON OBS.CONCEPT_CD = MAP.CONCEPT_CD AND OBS.START_DATE BETWEEN '01-01-2020' AND '12-31-2020'
    ) AS ONE_YEAR,
    
    ( SELECT COUNT(DISTINCT OBS.PATIENT_NUM) CNT
        FROM MAPPED_DOMAIN MAP
        JOIN OBSERVATION_FACT OBS ON OBS.CONCEPT_CD = MAP.CONCEPT_CD AND OBS.START_DATE BETWEEN '01-01-2016' AND '12-31-2020'
    ) AS FIVE_YEAR
;
COMMIT;
...
SELECT
  'uniq_pt_smoking' AS VARIABLE_NAME,
   ...
   ( SELECT COUNT(DISTINCT OBS.ENCOUNTER_NUM) CNT
        ...
        JOIN OBSERVATION_FACT OBS ON OBS.CONCEPT_CD = MAP.CONCEPT_CD AND OBS.START_DATE BETWEEN '01-01-2020' AND '12-31-2020'
        JOIN PATIENT_DIMENSION PAT ON PAT.PATIENT_NUM = OBS.PATIENT_NUM 
            AND PAT.BIRTH_DATE IS NOT NULL AND DATEDIFF(year, per.birth_datetime, vis.visit_start_date) > 12 
    ) AS ONE_YEAR,
